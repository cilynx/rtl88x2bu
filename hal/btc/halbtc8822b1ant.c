...
static void
halbtc8822b1ant_limited_tx(struct btc_coexist *btc, boolean force_exec,
...
static void
halbtc8822b1ant_limited_rx(struct btc_coexist *btc, boolean force_exec,
...
static void
halbtc8822b1ant_ccklock_action(struct btc_coexist *btc)
{
	struct coex_sta_8822b_1ant *coex_sta = &btc->coex_sta_8822b_1ant;
...
static void
halbtc8822b1ant_ccklock_detect(struct btc_coexist *btc)
{
	struct coex_sta_8822b_1ant *coex_sta = &btc->coex_sta_8822b_1ant;
...
static void
halbtc8822b1ant_set_tdma_timer_base(struct btc_coexist *btc, u8 type)
{
	struct coex_sta_8822b_1ant *coex_sta = &btc->coex_sta_8822b_1ant;
						  ...
						  u8 reason)
{
	...
	if (wifi_link_info_ext->is_all_under_5g || num_of_wifi_link == 0 ||
		...
		halbtc8822b1ant_limited_tx(btc, NM_EXCU, FALSE, FALSE);
		halbtc8822b1ant_limited_rx(btc, NM_EXCU, FALSE, TRUE, 64);
	} else if (wifi_link_info_ext->num_of_active_port > 1) {
		halbtc8822b1ant_low_penalty_ra(btc, NM_EXCU, TRUE, 30);
		halbtc8822b1ant_limited_tx(btc, NM_EXCU, FALSE, TRUE);
		halbtc8822b1ant_limited_rx(btc, NM_EXCU, FALSE, TRUE, 16);
	} else {
		halbtc8822b1ant_low_penalty_ra(btc, NM_EXCU, TRUE, 15);

		if (coex_sta->hid_exist || coex_sta->hid_pair_cnt > 0 ||
		    coex_sta->sco_exist) {
			halbtc8822b1ant_limited_tx(btc, NM_EXCU, TRUE, TRUE);
			halbtc8822b1ant_limited_rx(btc, NM_EXCU, FALSE, TRUE,
						   16);
		} else {
			halbtc8822b1ant_limited_tx(btc, NM_EXCU, TRUE, FALSE);
			halbtc8822b1ant_limited_rx(btc, NM_EXCU, FALSE, TRUE,
						   64);
		}
	}
...
static void halbtc8822b1ant_update_bt_link_info(struct btc_coexist *btc)
{
	...
	if (!(coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_CONNECTION)) {
		...
		if (coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_A2DP) {
		...
		if (coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_HID) {
		...
		if (coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_SCO_ESCO) {
	...
	if (coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_INQ_PAGE) {
		coex_dm->bt_status = BT_8822B_1ANT_BSTATUS_INQ_PAGE;
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], BtInfoNotify(), BT Inq/page!!!\n");
	} else if (!(coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_CONNECTION)) {
		coex_dm->bt_status = BT_8822B_1ANT_BSTATUS_NCON_IDLE;
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
	...
	} else if (((coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_SCO_ESCO) ||
		    (coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_SCO_BUSY)) &&
		   (coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_ACL_BUSY)) {
		coex_dm->bt_status = BT_8822B_1ANT_BSTATUS_ACL_SCO_BUSY;
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
	...
	} else if (coex_sta->bt_info_lb2 & BT_INFO_8822B_1ANT_B_ACL_BUSY) {
		coex_dm->bt_status = BT_8822B_1ANT_BSTATUS_ACL_BUSY;
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], BtInfoNotify(), BT ACL busy!!!\n");
	} else {
...
static u32
halbtc8822b1ant_wait_indirect_reg_ready(struct btc_coexist *btc)
{
	u32 delay_count = 0;
...
void halbtc8822b1ant_ltecoex_enable(struct btc_coexist *btc, boolean enable)
{
	...
	val = (enable) ? 1 : 0;
	/* 0x38[7] */
	halbtc8822b1ant_write_indirect_reg(btc, 0x38, BIT(7), val);
}
...
static void
halbtc8822b1ant_set_gnt_bt(struct btc_coexist *btc, u8 state)
{
	switch (state) {
	case BTC_GNT_SET_SW_LOW:
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0xc000, 0x1);
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0x0c00, 0x1);
		break;
	case BTC_GNT_SET_SW_HIGH:
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0xc000, 0x3);
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0x0c00, 0x3);
		break;
	case BTC_GNT_SET_HW_PTA:
	default:
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0xc000, 0x0);
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0x0c00, 0x0);
		break;
	}
}

static void
halbtc8822b1ant_set_gnt_wl(struct btc_coexist *btc, u8 state)
{
	switch (state) {
	case BTC_GNT_SET_SW_LOW:
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0x3000, 0x1);
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0x0300, 0x1);
		break;
	case BTC_GNT_SET_SW_HIGH:
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0x3000, 0x3);
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0x0300, 0x3);
		break;
	case BTC_GNT_SET_HW_PTA:
	default:
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0x3000, 0x0);
		halbtc8822b1ant_write_indirect_reg(btc, 0x38, 0x0300, 0x0);
		break;
	}
}
...
void halbtc8822b1ant_table(struct btc_coexist *btc, boolean force_exec, u8 type)
{
	...
	case 0:
		...
		break;
	case 1:
		halbtc8822b1ant_set_table(btc, force_exec, 0x66555555,
	...
	case 5:
		...
		break;
	case 6:
		halbtc8822b1ant_set_table(btc, force_exec, 0x6a5a56aa,
	...
	case 9:
		...
		break;
	case 10:
		halbtc8822b1ant_set_table(btc, force_exec, 0xaaaaaaaa,
					  0xaaaaaaaa, break_table,
					  select_table);
		break;
	case 11:
	...
	case 13:
		...
		break;
	case 14:
		halbtc8822b1ant_set_table(btc, force_exec, 0x6a5a5aaa,
					  0x6a5a5aaa, break_table,
					  select_table);
		break;
	case 15:
		halbtc8822b1ant_set_table(btc, force_exec, 0xffff55ff,
					  0x6afa5afa, break_table,
					  select_table);
		break;
	case 16:
		halbtc8822b1ant_set_table(btc, force_exec, 0x55555555,
...
static void
halbtc8822b1ant_wltoggle_table(IN struct btc_coexist *btc,
...
static void
halbtc8822b1ant_tdma(struct btc_coexist *btc, boolean force_exec,
		     boolean turn_on, u32 tcase)
{
	...
	if (turn_on) {
		...
		case 1: /* not use */
			...
			break;
		case 3:
			halbtc8822b1ant_set_tdma(btc, 0x51, 0x35, 0x03, 0x10,
						 0x50);
			break;
		case 4:
		...
		case 6:
			...
			break;
		case 7:
			halbtc8822b1ant_set_tdma(btc, 0x51, 0x08, 0x03, 0x10,
						 0x54);
			break;
		case 8: /* not use */
			halbtc8822b1ant_set_tdma(btc, 0x51, 0x08, 0x03, 0x10,
			...
			break;
		case 10:
			halbtc8822b1ant_set_tdma(btc, 0x51, 0x08, 0x07, 0x10,
						 0x55);
			break;
		case 11:
		...
		case 16:
			...
			break;
		case 17:
			halbtc8822b1ant_set_tdma(btc, 0x61, 0x08, 0x03, 0x11,
						 0x14);
			break;
		case 18:
			halbtc8822b1ant_set_tdma(btc, 0x51, 0x30, 0x03, 0x10,
		...
		case 33:
			...
			break;
		case 34:
			halbtc8822b1ant_set_tdma(btc, 0x61, 0x45, 0x03, 0x11,
						 0x10);
			break;
		case 36:
			halbtc8822b1ant_set_tdma(btc, 0x61, 0x45, 0x03, 0x11,
		...
		case 37:
			...
			break;
		case 38:
			halbtc8822b1ant_set_tdma(btc, 0x65, 0x10, 0x03, 0x11,
						 0x11);
			break;
		case 39:
			halbtc8822b1ant_set_tdma(btc, 0x55, 0x08, 0x3, 0x10,
						 0x54);
			break;
		case 50:
			...
			break;
		case 51:
			halbtc8822b1ant_set_tdma(btc, 0x51, 0x08, 0x3, 0x30,
						 0x54);
			break;
		case 105:
		...
		halbtc8822b1ant_write_scbd(btc, BT_8822B_1ANT_SCBD_TDMA, FALSE);
				     ...
				     u8 pos_type)
{
	...
	if (rfe_type->ext_switch_type == BT_8822B_1ANT_SWITCH_USE_SPDT) {
		switch (ctrl_type) {
		default:
		case BT_8822B_1ANT_CTRL_BY_BBSW:
			...
			if (pos_type == BT_8822B_1ANT_TO_S0WLG_S1BT) {
				if (rfe_type->rfe_module_type != 0x4 &&
				    rfe_type->rfe_module_type != 0x2)
					val = 0x3;
				else
				  ...
				  u8 phase)
{
	...
	coex_sta->is_2g_freerun =
		((phase == BT_8822B_1ANT_PHASE_2G_FREERUN) ? TRUE : FALSE);
	...
	case BT_8822B_1ANT_PHASE_INIT:
		...
		halbtc8822b1ant_set_gnt_wl(btc, BTC_GNT_SET_SW_LOW);
	...
	case BT_8822B_1ANT_PHASE_WONLY:
		...
		halbtc8822b1ant_set_gnt_bt(btc, BTC_GNT_SET_SW_LOW);
	...
	case BT_8822B_1ANT_PHASE_2G_FREERUN:
	case BT_8822B_1ANT_PHASE_2G_WLBT:
	...
	case BT_8822B_1ANT_PHASE_2G_BT:
...
#endif
		{
			...
			/* Set GNT_WL to PTA */
			halbtc8822b1ant_set_gnt_wl(btc, BTC_GNT_SET_HW_PTA);
		}
	...
	if (phase == BT_8822B_1ANT_PHASE_WOFF) {
		...
		ctrl_type = BT_8822B_1ANT_CTRL_BY_BT;
		pos_type = BT_8822B_1ANT_TO_NOCARE;
	} else if (phase == BT_8822B_1ANT_PHASE_MCC) {
	...
	hallbtc8822b1ant_set_ant_switch(btc, force_exec, ctrl_type, pos_type);
...
static u8 halbtc8822b1ant_action_algorithm(struct btc_coexist *btc)
{
	...
	case 0:
		...
		BTC_TRACE(trace_buf);
		algorithm = BT_8822B_1ANT_COEX_UNDEFINED;
		break;
	case 1:
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
...
static void halbtc8822b1ant_action_bt_relink(struct btc_coexist *btc)
{
	...
	if (coex_sta->gl_wifi_busy) {
		...
		if (bt_link_info->hid_only)
			halbtc8822b1ant_tdma(btc, NM_EXCU, TRUE, 20
					     | TDMA_4SLOT);
		else
			halbtc8822b1ant_tdma(btc, NM_EXCU, TRUE, 20);
	} else {
...
static void halbtc8822b1ant_action_bt_idle(struct btc_coexist *btc)
{
	...
	if (coex_dm->bt_status == BT_8822B_1ANT_BSTATUS_NCON_IDLE &&
	    BTC_RSSI_HIGH(wifi_rssi_state) && coex_sta->gl_wifi_busy)
...
/* #ifdef PLATFORM_WINDOWS*/
	...
	if (!coex_sta->gl_wifi_busy) {
		...
		if (coex_dm->bt_status == BT_8822B_1ANT_BSTATUS_NCON_IDLE &&
		    btc->wifi_link_info.link_mode == BTC_LINK_ONLY_STA) {
...
static void halbtc8822b1ant_action_bt_inquiry(struct btc_coexist *btc)
{
	...
	if (coex_sta->is_wifi_linkscan_process ||
		...
		halbtc8822b1ant_table(btc, NM_EXCU, 23);
		halbtc8822b1ant_tdma(btc, NM_EXCU, TRUE, 19);
	} else {
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
...
void halbtc8822b1ant_action_bt_sco_hid_busy(struct btc_coexist *btc)
{
	...
	if (bt_link_info->sco_exist) {
		...
		halbtc8822b1ant_table(btc, NM_EXCU, 16);
...
static void halbtc8822b1ant_action_bt_acl_busy(struct btc_coexist *btc)
{
	...
	if (btc->board_info.customer_id == RT_CID_LENOVO_CHINA &&
		...
		if (BTC_RSSI_LOW(wifi_rssi_state2)) {
			...
			halbtc8822b1ant_set_bt_tx_power(btc, NM_EXCU, 0);
			halbtc8822b1ant_table(btc, NM_EXCU, 16);
			halbtc8822b1ant_tdma(btc, NM_EXCU, TRUE, 8);
		}
		return;
	}
	...
	if (bt_link_info->a2dp_exist && coex_sta->is_bt_a2dp_sink) {
		...
		if (wifi_busy)
		   ...
		   bt_link_info->a2dp_exist) { /* RCU + A2DP */
		halbtc8822b1ant_table(btc, NM_EXCU, 16);
...
static void halbtc8822b1ant_action_wifi_connected(struct btc_coexist *btc)
{
	...
	coex_dm->cur_algorithm = halbtc8822b1ant_action_algorithm(btc);
	...
	if (coex_sta->is_setup_link || coex_sta->bt_relink_downcount != 0) {
		...
		if (coex_sta->gl_wifi_busy) {
			if (coex_sta->wl_rx_rate <= 3 ||
			    coex_sta->wl_rts_rx_rate <= 3)
				halbtc8822b1ant_table(btc, NM_EXCU, 27);
			else if (coex_sta->bt_418_hid_exist ||
				 coex_sta->bt_ble_hid_exist)
				halbtc8822b1ant_table(btc, NM_EXCU, 28);
			else
				halbtc8822b1ant_table(btc, NM_EXCU, 25);
		} else {
			halbtc8822b1ant_table(btc, NM_EXCU, 25);
...
static void halbtc8822b1ant_action_wifi_scc2g(struct btc_coexist *btc)
{
	struct coex_sta_8822b_1ant *coex_sta = &btc->coex_sta_8822b_1ant;
	...
	if (coex_sta->num_of_profile == 1) {
		if (coex_sta->hid_exist || coex_sta->sco_exist) {
			...
			halbtc8822b1ant_tdma(btc, NM_EXCU, TRUE, 51);
		} else { /* PAN or OPP */
			halbtc8822b1ant_table(btc, NM_EXCU, 11);
			halbtc8822b1ant_tdma(btc, NM_EXCU, TRUE, 50);
		}
...
halbtc8822b1ant_action_wifi_multiport2g(struct btc_coexist *btc)
{
	...
	if (coex_sta->bt_disabled) {
		...
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
			    "[BTCoex], wifi_multiport2g, BT profile!!\n");
		BTC_TRACE(trace_buf);

		switch (btc->wifi_link_info.link_mode) {
		case BTC_LINK_2G_SCC_GC_STA:
		case BTC_LINK_2G_SCC_GO_STA:
			ant_pase = BT_8822B_1ANT_PHASE_2G;
			halbtc8822b1ant_set_ant_path(btc, BTC_ANT_PATH_AUTO,
						     NM_EXCU, ant_pase);
			halbtc8822b1ant_action_wifi_scc2g(btc);
			break;
		case BTC_LINK_ONLY_GO:
		case BTC_LINK_ONLY_GC:
...
static void halbtc8822b1ant_run_coex(struct btc_coexist *btc, u8 reason)
{
	...
	if (wifi_link_info_ext->num_of_active_port > 1 ||
		...
		if (btc->wifi_link_info.link_mode == BTC_LINK_ONLY_GO)
			coex_sta->wl_coex_mode = BT_8822B_1ANT_WLINK_2GGO;
		else if (btc->wifi_link_info.link_mode == BTC_LINK_ONLY_GC)
			coex_sta->wl_coex_mode = BT_8822B_1ANT_WLINK_2GGC;
		else
	...
	if (coex_sta->bt_a2dp_vendor_id == 0x4c && coex_sta->is_bt_multi_link)
		halbtc8822b1ant_write_scbd(btc, BT_8822B_1ANT_SCBD_CQDDR,
					   FALSE);
	else
		halbtc8822b1ant_write_scbd(btc, BT_8822B_1ANT_SCBD_CQDDR,
					   TRUE);
					   ...
					   boolean back_up, boolean wifi_only)
{
	...
	halbtc8822b1ant_init_coex_var(btc);
	...
	if (btc->wl_rf_state_off) {
		...
		halbtc8822b1ant_write_scbd(btc, BT_8822B_1ANT_SCBD_ALL, FALSE);
...
void ex_halbtc8822b1ant_lps_notify(struct btc_coexist *btc, u8 type)
{
	...
	if (type == BTC_LPS_ENABLE) {
		...
		if (coex_sta->force_lps_ctrl) { /* LPS No-32K */
			...
			halbtc8822b1ant_run_coex(btc, BT_8822B_1ANT_RSN_LPS);
		}
				       ...
				       u8 length)
{
	...
	if (rsp_source == BT_8822B_1ANT_INFO_SRC_BT_SCBD) {
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
	...
	if (coex_sta->bt_create_connection) {
		...
		if (coex_sta->is_wifi_linkscan_process ||
			...
			halbtc8822b1ant_write_scbd(btc, type_is_scan, TRUE);
		} else {
			halbtc8822b1ant_write_scbd(btc, type_is_scan, FALSE);
		}
	} else {
					      ...
					      u8 btc_rate_id)
{
	...
	halbtc8822b1ant_ccklock_detect(btc);
}
